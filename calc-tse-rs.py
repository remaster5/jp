...
LIST_FILENAME = "tse-list.csv"
TARGET = 'TSE'
DATA_DIR_ROOT = "DATA"

print("TSE 리스트를 가져옵니다.")
tse_list = fdr.StockListing(TARGET)
tse_list.to_csv(LIST_FILENAME)

print(tse_list.shape)
...
result_file_path = os.path.join(posts_dir, f"{date}-tse-rs.markdown")
...
with open(result_file_path, "w") as f:
    ...
    f.write(now.strftime('title: "TSE 상대강도 %Y년 %-m월 %-d일"\n'))
...
result_file_path = os.path.join(
    posts_dir, f"{date}-tse-trend-template.markdown")
...
with open(result_file_path, "w") as f:
    ...
    '''
    f.write(textwrap.dedent(header_start))
    f.write(now.strftime('title: "TSE 미너비니 트렌드 템플릿 %Y년 %-m월 %-d일"\n'))
    f.write(now.strftime("date: %Y-%m-%d %H:%M:%S +0900\n"))
    header_end = '''\
